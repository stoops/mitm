/* gcc -DECK -Wall -O3 -fPIC -shared -o key.o lib/key.c -lcrypto */
...
struct keyp xchg(int sock, char *skey, char mode, int indx)
{
	...
	for (int z = 0; z < 2; ++z)
	{
		if (((mode == 'c') && (z == 0)) || ((mode == 's') && (z == 1)))
		{
			...
			if (slen < 1) { /* no-op */ }
			printf("%s SKEY [%d][%c] {S} <%d><%lu>\n", date(), indx, mode, rlen, snum);
		}

		if (((mode == 's') && (z == 0)) || ((mode == 'c') && (z == 1)))
		{
			for (int x = 0; (x < KTRY) && (auth != 1); ++x)
			{
				...
				{
					rlen = rall(sock, 0, data);
					if (rlen > 0)
					{
						pntr = (data->buff + OFFS); rlen = (rlen - OFFS);
						bcopy(pntr, encr, rlen);
						bzero(decr, KEXL);
						slen = ciph(decr, encr, rlen, &dkey, 'd');
						if ((slen == mlen) && (memcmp(decr, "auth-", klen) == 0) && (memcmp(decr, init, mlen) != 0))
						{
							printf("%s SKEY [%d][%c] {R} <%d><%lu>\n", date(), indx, mode, rlen, snum);
							bcopy(decr, inpt, mlen);
							auth = 1;
						}
					}
					else { printf("%s SKEY [%d][%c] {L} <%d><%lu>\n", date(), indx, mode, rlen, snum); }
				}
			}
		}
	}
	...
	if (auth == 1)
	{
		...
		keys.klen = ARCK;
		printf("%s SKEY [%d][%c] {X} <0x%s>\n", date(), indx, mode, outh);
	}
	else
	{
		printf("%s SKEY [%d][%c] {E} <%lu>\n", date(), indx, mode, snum);
	}
...
struct keyp xchg(int sock, char *skey, char mode, int indx)
{
	...
	csxn = NULL; csyn = NULL;
	scxn = (uchar *)strdup("0"); scyn = (uchar *)strdup("0");
	sexn = (uchar *)strdup("0"); seyn = (uchar *)strdup("0");
	shsh = (uchar *)strdup("0");
	if (leng == maxl)
	{
		buff[512] = 0; bzero(buff + 512, 16); pnts = (char *)buff;
		temp = strchr(pnts, ' '); if (temp != NULL) { *temp = 0; free(scxn); scxn = (uchar *)strdup(pnts); pnts = (temp + 1); }
		temp = strchr(pnts, ' '); if (temp != NULL) { *temp = 0; free(scyn); scyn = (uchar *)strdup(pnts); pnts = (temp + 1); }
		temp = strchr(pnts, ' '); if (temp != NULL) { *temp = 0; free(sexn); sexn = (uchar *)strdup(pnts); pnts = (temp + 1); }
		temp = strchr(pnts, ' '); if (temp != NULL) { *temp = 0; free(seyn); seyn = (uchar *)strdup(pnts); pnts = (temp + 1); }
		temp = strchr(pnts, ' '); if (temp != NULL) { *temp = 0; free(shsh); shsh = (uchar *)strdup(pnts); pnts = (temp + 1); }
